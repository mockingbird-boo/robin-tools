...
public class ApiSecurityAspect {
            ...
            switch (strategy) {
                ...
                case TOKEN -> {
                    ...
                    if (StringUtils.isBlank(token)) {
                        throw new ApiSecurityException(ResponseCode.BAD_REQUEST.getCode(), "幂等防重 TOKEN 不能为空");
                    }
                    // 有幂等令牌就删，成功删掉即非重复提交
    ...
    private void validSignature(String signature) {
        if (StringUtils.isBlank(signature)) {
            throw new ApiSecurityException(ResponseCode.BAD_REQUEST.getCode(), "数字签名不能为空");
        }
    }
